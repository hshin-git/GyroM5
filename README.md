...
- M5StickCにスケッチ[GyroM5.ino](GyroM5/GyroM5.ino)をインストールしてRCユニットと接続すれば完成です。
- 最新v2スケッチ[GyroM5v2.ino](GyroM5v2/GyroM5v2.ino)は、スマートフォン連携、サーボ周波数可変等の新機能に対応です。
...
- (-v2) ドリフト走行検出機能 <br>ドリフト走行（回頭方向と操舵方向が逆）を検出してLCD背景色を変更します。
- (+v2) スマホ連携機能 <br>WiFi通信によりスマートフォン経由で各種設定できます。
- (+v2) サーボPWM可変機能 <br>サーボPWM周波数を50-400Hzの範囲で変更できます。
- (+v2) グラフ表示機能 <br>PID制御データ（目標値、入出力）をグラフ表示します。
- (+v2) データ保存機能 <br>WiFi通信によりPID制御データをダウンロードできます。
- (+v2) 鉛直検出機能 <br>起動時に鉛直方向を自動検出できシャーシ固定が自由です。
- (+v2) 時刻表示機能 <br>現在時刻をLCD画面に表示します。

キーワードの"PID制御"は、
汎用性の高いフィードバック制御アルゴリズムであり、
調整可能パラメータ4個で自由自在のセッティングを実現します。
...
![GyroM5v1](https://user-images.githubusercontent.com/64751855/124377619-c07e7080-dce7-11eb-90ce-ff4f0edb7783.png)
...
![GyroM5v2](https://user-images.githubusercontent.com/64751855/124377638-d9872180-dce7-11eb-8efa-6dfadb6e490c.png)
