...
- 標準的パソコン <br>ArduinoIDEをインストールでき、USBインターフェイスを備えるスケッチ書き込み用パソコンです。
...
たとえば電源や信号の電圧レベルが6Vを超える場合、レベルコンバータを省略するとM5StickCが破損（不可逆に故障）する恐れがあります。
...
内蔵バッテリの充電不足を除いた場合、電源投入時点のGPIOピン（おそらくG0ピンの）電圧によりM5StickC起動の成否が決まるようです。
...
- 測定ゲイン: kg = KG/0.5 <br>大きくするとヨーレートに敏感となり、ステアリングに対する回頭角速度は小さくなります。
...
### リアルタイム性
GyroM5は、制御周期が約50Hz、入出力処理が20ms以内のリアルタイムシステムです。

- 「約50Hz」の理由は、PWM幅計測の標準関数pulseIn(...)がブロックするので、複数CH入力時に20msを超えるからです。
