...
- M5StickCにスケッチGyroM5.inoを転送してRCメカと接続するだけで、RCカー用ジャイロGyroM5として機能します。
...
# DEMO
GyroM5搭載のタミヤ純製RCカー（SU-01シャーシとファインスペック）がRWDドリフト走行する様子です。
...
- フィードバック制御機能 <br>車体ヨーレートを目標にフィードバック制御（PID制御）できます。
...
RCメカの接続方法、M5StickCの起動方法、PID制御パラメータの解釈は後半を参照ください。
...
- 制御量: y = kg*wz = RCカーの車体ヨーレート
- 目標値: r = ch1_in = RC受信機からのCH1入力
- 操作量: u = ch1_out = RCサーボへのCH1出力

つまり受信機からのCH1入力を車体ヨーレートの目標値と解釈して、
