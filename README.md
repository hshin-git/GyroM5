...
- GyroM5（じゃいろ・えむご）は、RCカーのステアリング補助用ジャイロをM5StickCで自作するためのOSSです。
...
- ホビー用RCカー　<br>標準的なRCメカを搭載して、受信機とステアリング用サーボを3線（-+S）で接続するRCカーです。
...
GyroM5利用時のM5StickCのGPIO端子とRCメカ（受信機、サーボ）端子との接続方法は下記の通りです。
...
M5StickCのGPIO端子は、プログラム修正で変更可能ですが、G0端子を入力用に用いるのは避けたほうが良いです。
G0端子は、内部的にプルアップされているようで、電源投入時にG0端子がLレベルだとM5StickCが起動しないことがあります。
