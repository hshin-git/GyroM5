...
- 本格的なPID制御アルゴリズムの採用により、アシスト動作を自由自在にセッティングできます。
...
|制御画面|ボタン[A],[B]|PID制御状態（RCユニット入出力、IMU入力、PIDパラメータなど）の表示|
|PID設定|ボタン[A],[B]|PID制御パラメータ、操舵CH1エンドポイントの保存（電源オフ後も残る）|
|CH3設定|ボタン[B]|CH3入力とPID制御パラメータの対応付けの選択|
