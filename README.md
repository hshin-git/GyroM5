...
- GyroM5は、ドリフトRCカーのステアリングジャイロを開発ボードM5StickCで自作するためのOSSです。
- 本格的なPID制御アルゴリズムの採用により、ステアリングのアシスト機能を高い自由度で設定できます。
- M5StickCにv1ファームウェア[GyroM5.ino](GyroM5/GyroM5.ino)をインストールしてRCユニットと接続すれば完成です。
- (2021-07-04) 新型のv2ファームウェア[GyroM5v2.ino](GyroM5v2/GyroM5v2.ino)は、スマートフォン連携、サーボ周波数可変等に対応です。
...
新型のv2ファームウェアは、ジャイロ設定にスマホを用いますが、v1ファームウェアの上位互換となる以下の機能を備えています。
つまりv2ファームウェアは、RCカーの性能をv1スケッチより引き出して、RCカーを楽しくする機能満載で、スマホユーザにお勧めです。

- スマホ連携機能 <br>WiFi通信によりスマートフォン経由でジャイロ設定が可能です。
- サーボPWM可変機能 <br>サーボ用のPWM周波数とPID制御周期を50Hz〜400Hzの範囲で変更できます。
- グラフ表示機能 <br>PID制御データ（目標値、入出力）をLCD画面にグラフ表示できます。
- データ保存機能 <br>WiFi通信によりPID制御データをCSVファイルでダウンロードできます。
- 鉛直検出機能 <br>起動時に鉛直方向を自動検出するので、ジャイロのシャーシ固定方向に制約がありません。
...
- 「HOME」が、ホーム画面となり、この状態からボタン[A]で「CONF」へ、ボタン[B]で「CH3G」へ遷移します。
- 「CONF」は、PID制御パラメータ、操舵CH1エンドポイントをメニューから選んで設定します。
- 「CH3G」は、リモートゲイン調整機能の設定で、CH3とパラメータの対応を指定します。
- 他の状態の場合、ボタン操作が画面に表示してあるか、無操作のタイムアウトでホーム画面「HOME」へ戻ります。
- パラメータ設定は、送信機からステアリングCH1で数値を送り、ボタン[A]で保存（ボタン[B]で取消）します。
- ジャイロ起動直後の「CH3G」は、PIDパラメータ表の参照が有効で、CH3リモートゲイン指定が無効です。
...
- 「ENDS」は、ステアリングCH1のエンドポイント（ステアリング用サーボ角度の最小値と最大値）を設定します。
